...
export class Search extends Component {
  ...
  handleChooseGame = async (gameSelected) => {
    ...
    try {
      const result = await api.fetchBoardGames(`thing?id=${id}`);
  ...
  addGameToFavorites = async() => {
    try {
      await db.doAddFavoriteData(this.props.user.uid, this.state.game);
  ...
  displayAllGames = () => {
    if (this.state.games.length)  {
      return (this.state.games.map((game, index) => 
        <h3 key={ index } onClick={() => this.handleChooseGame(game)}>
          { game.name }
        </h3> )
      );
    } else {
  ...
  displayGame = () => {
    return this.state.game.thumbnail 
      ?  <article className="game-description">
        <button id={this.state.gameAdded} onClick={this.addGameToFavorites}>
        </button>
